...
from pysqldbm.resources.ddl import DDLs
...
class Revisions(BaseResources):
    def __init__(self, client: RestClient, project_id: str):
        super().__init__(client)
        self._project_id = project_id

    def get(self, revision_id: str) -> Revision:
        yield from self._client.get_one(
            f"revisions/{self._project_id}/revisions/{revision_id}", Revision
        )
